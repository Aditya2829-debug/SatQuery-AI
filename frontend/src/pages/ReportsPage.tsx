...
export const ReportsPage: React.FC = () => {
  const { history, currentResult } = useSatStore();

  const handleDownloadJSON = (result: typeof currentResult) => {
    if (!result) return;
    ...
    downloadAnchor.setAttribute(
      'download',
      `SatQuery_Intelligence_${result.id}.json`
    );
  ...
  const handleDownloadPDF = () => {
    ...
    exportAnalysisPDF(reportQuery, currentResult);
  };
      ...
      {!currentResult ? (
        <div className="py-12 text-center text-xs text-slate-400">
          {history.length === 0
            ...
            : 'Select or complete an analysis in the console to view its generated report.'}
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex flex-col justify-between gap-4 rounded-xl border border-slate-200 p-4 sm:flex-row sm:items-center dark:border-slate-700">
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                ...
                <span className="flex items-center space-x-1 text-xs font-semibold text-success">
                  ...
                  <span>
                    Veracity: {(currentResult.confidence * 100).toFixed(1)}%
                  </span>
                </span>
              </div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Model: {currentResult.metadata.modelUsed} | Latency:{' '}
